...
class supplierClass:
    def __init__(self,root):
        ...
        txt_search=Entry(self.root,textvariable=self.var_searchtxt,font=("times new roman",15),bg="#BCD2E8").place(x=800,y=80,width=160)
        btn_search=Button(self.root,text="Search",command=self.search,font=("goudy old style",15),bg="#2E5984",fg="white",cursor="hand2").place(x=980,y=79,width=100,height=28)
        ...
        txt_supplier_invoice=Entry(self.root,textvariable=self.var_sup_invoice,font=("times new roman",15),bg="#BCD2E8").place(x=180,y=80,width=180)
        
        #row2

        lbl_Name=Label(self.root,text="Name",font=("times new roman",15),bg="white").place(x=50,y=120)
        txt_Name=Entry(self.root,textvariable=self.var_name,font=("times new roman",15),bg="#BCD2E8").place(x=180,y=120,width=180)
        
        #row3

        lbl_contact=Label(self.root,text="Contact",font=("times new roman",15),bg="white").place(x=50,y=160)
        txt_contact=Entry(self.root,textvariable=self.var_contact,font=("times new roman",15),bg="#BCD2E8").place(x=180,y=160,width=180)
       
        #row4

        lbl_desc=Label(self.root,text="Description",font=("times new roman",15),bg="white").place(x=50,y=200)
        self.txt_desc=Text(self.root,font=("times new roman",15),bg="#BCD2E8")
        ...
        btn_clear=Button(self.root,text="Clear",command=self.clear,font=("goudy old style",15),bg="#607d8b",fg="white",cursor="hand2").place(x=420,y=370,width=110,height=35)
        btn_delete=Button(self.root,text="Delete",command=self.delete,font=("goudy old style",15),bg="#f44336",fg="white",cursor="hand2").place(x=540,y=370,width=110,height=35)
    ...
    def add(self):
        ...
        try:
            if self.var_sup_invoice.get()=="" or self.var_name.get()=="":
                ...
                row = cur.fetchone()
                if row != None:
                    messagebox.showerror("Error","Invoice no. already Exists, try a different",parent = self.root)
                else :
